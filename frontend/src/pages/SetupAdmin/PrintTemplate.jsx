...
const DEFAULT_LABELS = {
  "Hóa đơn": {
    ...
    footerMsg: {
      text: "Cảm ơn quý khách đã đến chơi tại Billiards Lục Lọi!",
      top: 360,
      left: 0,
      width: 380,
      align: "center",
...
const MOCK_DATA_MAP = {
  "Hóa đơn": {
    TENBAN: "Bàn 4",
    TENCUAHANG: "Billiards Lục Lọi",
    DIACHI: "123 Đường Số 1, TP.HCM",
    SDT: "0901234567",
    MAHOADON: "HD12345",
    NGAY: "05/03/2026",
    THOIGIAN: "21:35",
    TENNGUOIDUNG: "Thành Lợi",
    GIOBATDAU: "20:30",
    GIOKETTHUC: "21:35",
    SOGIOPHUT: "01:05",
    items: [
    ...
    TONGTIENGIO: "60,000đ",
    TONGTIENHANG: "40,000đ",
    GIAMGIA: "0",
    TONGTHANHTOAN: "100,000đ",
  },
  "Trả hàng": {
    TENCUAHANG: "Billiards Lục Lọi",
    DIACHI: "123 Đường Số 1, TP.HCM",
  ...
  "Nhập hàng": {
    ...
    SDT: "0901234567",
    MAPHIEUNHAP: "PN00123",
    THOIGIAN: "05/03/2026 09:15",
    TENNHANVIEN: "Thành Lợi",
    TENNCC: "Đại lý bia Tiger",
    items: [
      {
        TENHANGHOA: "Bia Tiger",
        SOLUONGNHAP: 2,
        DONGIA: "350,000",
        THANHTIEN: "700,000",
      },
      {
        TENHANGHOA: "Bia Heineken",
        SOLUONGNHAP: 1,
        DONGIA: "420,000",
        THANHTIEN: "420,000",
      },
  ...
  "Xuất kho": {
    ...
    SDT: "0901234567",
    MAPHIEUXUAT: "PX00123",
    THOIGIAN: "05/03/2026 14:00",
    TENNHANVIEN: "Thành Lợi",
    LYDO: "Xuất dùng nội bộ",
    items: [
      {
        TENHANGHOA: "Bóng Billiards",
        SOLUONGXUAT: 2,
        DONGIA: "100,000",
        THANHTIEN: "200,000",
      },
  ...
  "Nhập kho": {
    ...
    SDT: "0901234567",
    MAPHIEUNHAP: "NK00125",
    THOIGIAN: "05/03/2026 16:45",
    TENNHANVIEN: "Thành Lợi",
    items: [
      {
        TENHANGHOA: "Cơ Billiards",
        SOLUONGNHAP: 10,
        DONGIA: "500,000",
        THANHTIEN: "5,000,000",
      },
...
const DraggableItem = ({
  ...
  React.useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      const newLeft = Math.max(
        0,
        Math.min(380 - (label.width || 50), e.clientX - startPos.x),
      );
  ...
  const style = {
    ...
    fontFamily:
      id === "title" || id.includes("lblInvoice") ? "inherit" : "sans-serif",
  };
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={
        !isPreview ? "hover:border-blue-400 group transition-all rounded" : ""
      }
    >
      {isPreview ? (
    ...
    const fetchTemplate = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:5000/api/templates/${activeTab}`,
        );
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === "ArrowUp") {
        ...
        handleLabelDrag(selectedId, {
          left: Math.min(maxWidth, current.left + step),
        });
        e.preventDefault();
      } else if (
        e.key === "Delete" ||
        (e.key === "Backspace" && !e.target.hasAttribute("contenteditable"))
      ) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          Swal.fire(
            "Đã khôi phục",
            "Bản in đã được đưa về trạng thái mặc định chuyên nghiệp.",
            "success",
          );
        } catch (e) {
  ...
  return (
    <div className="h-screen bg-slate-50 font-sans text-slate-800 flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto px-8 py-6 bg-white min-h-0">
        <h1 className="text-2xl font-bold text-slate-800 mb-6 font-sans">
          Mẫu in
        </h1>
        ...
        <div className="flex flex-col xl:flex-row gap-8 min-h-[600px] h-auto pb-10">
          <div className="flex-1 bg-[#F4F6F8] relative border border-slate-200 rounded">
            <div className="flex justify-between items-center p-4">
              <h3 className="font-bold text-slate-600 text-[13px] font-sans">
                Mẫu in ({activeTab})
              </h3>
              <button
                onClick={handleReset}
                className="bg-white border border-slate-200 px-3 py-1 text-[11px] font-sans font-bold rounded shadow-sm text-slate-600 hover:bg-slate-50 cursor-pointer transition-colors"
              >
                Khôi phục gốc
              </button>
            </div>
            ...
            <div className="absolute bottom-4 right-4 z-10 flex gap-2">
              <button
                onClick={handleSave}
                className="bg-[#4154F1] hover:bg-blue-700 font-sans font-bold text-white flex items-center gap-2 px-4 py-2 text-[13px] rounded shadow cursor-pointer transition-colors"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
          ...
          <div className="flex-1 bg-[#E1EDFA] relative border border-slate-200 rounded">
            <div className="flex justify-end items-center p-4">
              <h3 className="font-bold text-slate-800 text-[13px] font-sans">
                Xem trước mẫu in
              </h3>
            </div>
