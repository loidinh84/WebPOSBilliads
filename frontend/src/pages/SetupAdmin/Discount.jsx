...
export default function Discount() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      Swal.fire(
        "Lỗi",
        "Không thể lưu. Kiểm tra kết nối hoặc cơ sở dữ liệu.",
        "error",
      );
    }
  };
  ...
  const toggleStatus = async (discount) => {
    try {
      ...
      if (updatedData.NGAYBATDAU)
        updatedData.NGAYBATDAU = updatedData.NGAYBATDAU.split("T")[0];
      if (updatedData.NGAYKETTHUC)
        updatedData.NGAYKETTHUC = updatedData.NGAYKETTHUC.split("T")[0];

      await axios.put(
        `http://localhost:5000/api/discounts/${discount.MAKHUYENMAI}`,
        updatedData,
      );
      fetchDiscounts();
    } catch (error) {
  ...
  const filteredDiscounts = discounts.filter(
    (d) =>
      ...
      d.TENKHUYENMAI?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      d.MACHITETKHOAN?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      d.MACHITIETKHUYENMAI?.toLowerCase().includes(searchTerm.toLowerCase()),
  );
  ...
  return (
    <div className="h-screen bg-slate-50 font-sans text-slate-800 flex flex-col overflow-hidden">
      ...
      <main className="flex-1 p-4 bg-white  max-w-[1600px] mx-auto gap-4">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F4F6F8] text-slate-600 text-sm border-b border-slate-200">
                ...
                <th className="p-4 font-bold">Tên khuyến mãi</th>
              ...
              {filteredDiscounts.length > 0 ? (
                filteredDiscounts.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      <td className="p-4 font-semibold text-[#4154F1]">
                        {item.MAKHUYENMAI}
                      </td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleSave} className="p-6">
                ...
                <div className="flex gap-4">
                  <div className="w-1/3">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none bg-slate-100 cursor-not-allowed text-slate-500"
                      placeholder={editingId ? "" : "Tự động tạo"}
                    />
                  </div>
                  <div className="w-2/3">
                    <label className="block text-sm font-bold text-slate-700 mb-1">
              ...
              <div className="mt-8 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 text-white bg-[#4154F1] hover:bg-blue-700 rounded-lg font-medium shadow transition-colors"
                >
                  Lưu lại
                </button>
              </div>
