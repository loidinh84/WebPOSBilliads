...
function Timesheet() {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      case "DUNG_GIO": return "bg-blue-600";
      case "TRE_SOM": return "bg-purple-600";
      case "THIEU": return "bg-red-500";
      case "CHUA_CHAM": return "bg-orange-500";
      ...
      <main className="max-w-[1600px] mx-auto p-4 flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center border border-gray-200 rounded-lg px-3 py-2 bg-gray-50 w-[300px] focus-within:bg-white transition-all shadow-inner">
            <img src={Icons.Search} alt="search" className="w-4 h-4 opacity-40 mr-3" />
            <input 
              type="text" 
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <img src={Icons.Calendar2} alt="view" className="w-4 h-4 opacity-60" />
              Xem theo ca
            </button>
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg font-bold hover:bg-gray-50 transition-all text-gray-600"
            >
              <img src={Icons.Calendar} alt="approve" className="w-4 h-4 opacity-60" />
              Duyệt chấm công
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden min-h-[600px] flex flex-col">
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100 text-[12px] font-bold text-gray-500 uppercase tracking-widest text-center">
                  <th className="p-4 text-left border-r border-gray-50 w-[200px]">Nhân viên</th>
                  ...
                  <th className="p-4 border-r border-gray-50 text-blue-500">Thứ bảy</th>
                  <th className="p-4 border-r border-gray-50 text-red-500">Chủ nhật</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  <tr>
                    <td colSpan="9" className="h-[400px] text-center">
                      <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <p className="font-bold text-gray-400">Đang tải bảng chấm công...</p>
                      </div>
                    </td>
                  </tr>
                ) : employees.length > 0 ? (
                    ...
                    <tr key={emp.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="p-4 border-r border-gray-50">
                        <div className="flex flex-col">
                          <span className="font-bold text-gray-900 text-[14px]">{emp.name}</span>
                          <span className="text-[11px] text-gray-400 font-bold uppercase">{emp.role}</span>
                        </div>
                      </td>
                      {weekDays.map(date => {
                        ...
                        return (
                          <td 
                            ...
                            onClick={() => openModal("MANUAL_ATTENDANCE", { 
                              staffId: emp.id, 
                              staffName: emp.name, 
                              date: date, 
                              dayData: dayData 
                          ...
                          >
                            {dayData && dayData.status !== "NGHI" ? (
                              <div className="flex flex-col items-center gap-1">
                                <div className={`w-2 h-2 rounded-full ${getStatusStyle(dayData.status)}`}></div>
                                ...
                                {dayData.salary > 0 && (
                                  <div className="text-[11px] font-black text-blue-900 mt-1">
                                    {new Intl.NumberFormat('vi-VN').format(dayData.salary)}đ
                                  </div>
                                )}
                                {dayData.discrepancy && (
                                  <div className={`text-[10px] font-bold mt-0.5 ${dayData.status === "TRE_SOM" || dayData.status === "THIEU" ? "text-red-500" : "text-orange-500"}`}>
                                    {dayData.discrepancy}
                                  </div>
                                )}
                              </div>
                            ) : (
                              <div className="text-[11px] text-gray-300 font-medium italic text-center">Nghỉ</div>
                            )}
                          </td>
                        );
                      })}
                  ...
                  <tr>
                    <td colSpan="9" className="h-[400px] text-center text-gray-400">
                      <div className="flex flex-col items-center justify-center gap-2">
                        <img src={Icons.User} alt="empty" className="w-12 h-12 opacity-10" />
                        <p className="font-medium">Không tìm thấy dữ liệu trong tuần này</p>
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="mt-auto border-t border-gray-100 bg-gray-50/30 p-4 flex justify-center items-center gap-8 text-[11px] font-bold text-gray-500 uppercase tracking-wider">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-blue-600 shadow-sm"></span> Đúng giờ
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-purple-600 shadow-sm"></span> Muộn/Sớm
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-red-500 shadow-sm"></span> Thiếu
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-orange-500 shadow-sm"></span> Chưa chấm
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-gray-200 shadow-sm"></span> Nghỉ làm
            </div>
          </div>
        </div>
      </main>
