  ...
  const fetchSchedules = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/weekly?startDate=${monday}&endDate=${sunday}`,
      );
  ...
  const handleSave = async (newValue) => {
    ...
    if (type === "EDIT_SHIFT") {
      ...
      try {
        ...
        const errorMsg =
          err.response?.data?.message || "Không thể cập nhật ca làm.";
        Swal.fire("Lỗi", errorMsg, "error");
      }
    } else if (type === "EDIT_SALARY") {
      Swal.fire(
        "Thông báo",
        "Tính năng chỉnh sửa lương đang được phát triển.",
        "info",
      );
    }
    closeModal();
                  ...
                  >
                    {emp.schedule[item.day] === "OFF" ? (
                      ...
                      <div className="flex flex-col items-center gap-1">
                        ...
                        >
                          {typeof emp.schedule[item.day] === "object" &&
                          emp.schedule[item.day] !== null
