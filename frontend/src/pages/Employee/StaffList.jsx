...
function StaffList() {
  ...
  const filteredEmployees = employees.filter((emp) => {
    ...
    const matchesStatus =
      filterStatus === "all" || 
      (filterStatus === "working" && (emp.TRANGTHAI === 1 || emp.TRANGTHAI === true || emp.TRANGTHAI === null || emp.TRANGTHAI === undefined)) ||
      (filterStatus === "off" && (emp.TRANGTHAI === 0 || emp.TRANGTHAI === false));

    // 3. Lọc theo Chức danh (Dropdown)
    // Cho phép khớp cả tên cũ (ví dụ: Quản lý) và tên mới (Quản lý (40k))
    const matchesRole =
      filterRole === "Chọn chức danh" ||
      (emp.CHUCVU && emp.CHUCVU.toLowerCase().includes(filterRole.toLowerCase()));
  ...
  const handleApproveRequest = async (reqId) => {
    try {
      ...
      Swal.fire("Thành công", "Đã duyệt yêu cầu và cập nhật trạng thái nhân viên!", "success");
      fetchEmployees();
      fetchLeaveRequests();
    } catch (err) {
