...
const renderModalContent = (
  ...
  switch (type) {
    ...
    case "EDIT_SALARY":
      return (
        <EditSalaryContent data={data} onSave={onSave} onClose={onClose} />
      );
    default:
      return (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                value={form.gender}
                onChange={handleChange}
                className="w-full bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-[14px] outline-none focus:border-[#5D5FEF] focus:ring-2 transition-all"
              >
...
const ImportDataContent = ({
  ...
  <div className="flex flex-col items-center justify-center py-10 px-4">
    ...
    <div
      onClick={() =>
        !isImporting && document.getElementById("excel-upload")?.click()
      }
...
const ExportDataContent = ({ onExport, format, setFormat }) => (
  <div className="space-y-6">
    <div className="p-6 bg-blue-50 rounded-2xl border border-blue-100 flex items-center gap-4">
      ...
      <div>
        <h4 className="text-[16px] font-bold text-blue-900">
          Sẵn sàng xuất lịch làm việc
        </h4>
        <p className="text-[13px] text-blue-700/70">
          Tất cả dữ liệu lịch làm việc trong tuần sẽ được đóng gói vào file{" "}
          {format.toUpperCase()}.
        </p>
      </div>
      <button
        ...
        {["xlsx", "pdf"].map((f, idx) => (
          <label
            ...
            className={`flex-1 flex items-center justify-center gap-3 p-4 bg-white border rounded-xl cursor-pointer transition-all ${
              format === f
                ? "border-[#5D5FEF] ring-2 ring-[#5D5FEF]/5"
                : "border-gray-100"
            }`}
          >
            <input
...
const ViewByShiftContent = () => {
  ...
  const fetchSchedules = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/schedule/by-shift`,
        {
  ...
  const getFilteredSchedules = () => {
    ...
    return allSchedules.filter((s) => {
      ...
      return (
        startHour >= currentShift.range[0] && startHour < currentShift.range[1]
      );
    });
  };
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-[400px] text-gray-400 px-10 text-center">
              ...
              <h3 className="text-[15px] font-bold text-gray-700 mb-2">
                Không tìm thấy ai trong ca này
              </h3>
              <p className="text-[13px] leading-relaxed max-w-[280px]">
                Hãy thử chọn ngày khác hoặc chuyển sang **"Full-day"** để xem
                tất cả nhân viên làm việc trong ngày {selectedDate}.
              </p>
              <button
...
const ApproveAttendanceContent = ({ onRefresh }) => {
  ...
  const handleEdit = (id) => {
    setRecords(
      records.map((r) => (r.id === id ? { ...r, isEditing: !r.isEditing } : r)),
    );
  };
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setRecords(
        records.map((r) =>
          r.id === id ? { ...r, isApproved: true, isEditing: false } : r,
        ),
      );
  ...
  const handleSaveEdit = async (record) => {
    try {
      ...
      setRecords(
        records.map((r) =>
          r.id === record.id ? { ...r, isApproved: true, isEditing: false } : r,
        ),
      );
      if (onRefresh) onRefresh();
    } catch (error) {
            ...
            {loading ? (
              ...
              <tr>
                <td
                  ...
                  className="px-6 py-10 text-center text-gray-400"
                >
                  Không có bản ghi nào chờ duyệt.
                </td>
              </tr>
            ) : (
              records.map((item) => (
                  ...
                  <td className="px-6 py-4">
                    {item.isEditing ? (
                      <input
                        ...
                        value={item.time}
                        onChange={(e) =>
                          handleChangeTime(item.id, e.target.value)
                        }
                      ...
                      <span
                        className={`${item.isApproved ? "text-green-700 font-medium" : "text-gray-500"}`}
                      >
                        {item.time}
                      </span>
                    )}
      ...
      <div className="p-5 bg-white rounded-xl border border-gray-100 shadow-sm">
        ...
        <div className="space-y-3">
          <div className="flex justify-between text-[14px]">
            ...
            <span className="font-bold text-gray-800">
              {data?.department || "Dịch vụ"}
            </span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-gray-500">Chức vụ</span>
            <span className="font-bold text-gray-800">
              {data?.role || data?.CHUCVU || "N/A"}
            </span>
          </div>
        </div>
      </div>
...
const EditShiftContent = ({ data, onSave, onClose }) => {
  ...
  useEffect(() => {
    try {
      ...
      const completed =
        typeof val === "object" && val !== null ? !!val?.isCompleted : false;
  ...
  return (
    <div className="py-4 space-y-6">
        ...
        <div className="p-4 bg-green-50 border border-green-100 rounded-xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] text-green-600 font-medium">
              Lịch sử chấm công đã được ghi nhận. Không thể thay đổi kế hoạch
              ban đầu.
            </p>
          </div>
        </div>
      ...
      <div
        className={`p-4 bg-white rounded-xl border border-gray-100 shadow-sm flex items-center justify-between ${isCompleted ? "opacity-60" : ""}`}
      >
        <div>
          ...
          <p className="text-[12px] text-gray-500">
            Chọn "Đang nghỉ" nếu không đi làm ngày này
          </p>
        </div>
        <button
      ...
      {!isOff && (
        <div
          className={`space-y-6 ${isCompleted ? "opacity-50 pointer-events-none" : ""}`}
        >
          <div>
  ...
  return (
    <div className="py-6 space-y-4">
      ...
      <p className="text-[13px] text-gray-500 bg-blue-50 p-3 rounded-lg border border-blue-100 italic">
        Lưu ý: Đây là lương dự kiến tính theo lịch trình, chưa bao gồm
        thưởng/phạt thực tế.
      </p>
    </div>
  );
  ...
  const handleSubmit = async () => {
    try {
      ...
      const msg =
        ...
        "Không thể lưu chấm công. Vui lòng thử lại.";
      alert(msg);
    }
  };
    ...
    <div className="py-4 space-y-6">
      <div className="p-4 bg-[#5D5FEF]/5 rounded-xl border border-[#5D5FEF]/10 flex justify-between items-center">
        ...
        <div className="text-right">
          ...
          <p className="text-[16px] font-bold text-[#5D5FEF]">
            {formatDateLabel(form.ngay)}
          </p>
        </div>
      </div>
...
const SalarySheetContent = ({ data, onClose, onSaveSuccess }) => {
  ...
  const handlePay = async () => {
    try {
      ...
      await axios.post("http://localhost:5000/api/salary/finalize", {
        payrolls: [updatedData],
      });
    ...
    <div className="py-4 space-y-6">
      <div className="p-4 bg-[#5D5FEF]/5 rounded-xl border border-[#5D5FEF]/10 space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            ...
            <p className="text-[18px] font-bold text-[#5D5FEF]">
              {data.KYTRALUONG}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
          ...
          <p className="text-[20px] font-extrabold text-[#5D5FEF]">
            {data.TONGGIOLAM}h
          </p>
        </div>
        <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
          ...
          <span
            className={`inline-block px-3 py-1 rounded-full text-[12px] font-bold ${data.TRANGTHAI === "Đã thanh toán" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
          >
            {data.TRANGTHAI}
          </span>
        </div>
      ...
      <div className="p-6 bg-white rounded-xl border border-gray-100 shadow-sm space-y-4">
        <div className="flex justify-between items-center text-[15px]">
          ...
          <span className="font-bold text-gray-800 text-[18px]">
            {new Intl.NumberFormat("vi-VN").format(data.TONGLUONG)}đ
          </span>
        </div>
        <div className="flex justify-between items-center text-[15px] pt-4 border-t border-gray-100">
          ...
          <span className="font-extrabold text-red-600 text-[24px]">
            {new Intl.NumberFormat("vi-VN").format(data.CONCANTRA)}đ
          </span>
        </div>
      </div>
