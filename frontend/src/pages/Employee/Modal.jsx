 ...
 */
const EmployeeModal = ({ isOpen, onClose, type, data, onAddEmployee, onUpdateEmployee, requests, onApproveRequest, onRejectRequest }) => {
  ...
  const getHeaderTitle = () => {
    switch (modalType) {
      ...
      case "EDIT": return `Chỉnh sửa: ${data?.name || "Nhân viên"}`;
      case "APPROVE_REQUESTS": return "Duyệt yêu cầu nhân viên";
      case "VIEW_BY_EMPLOYEE": return `Hồ sơ: ${data?.name || ""}`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50/30">
          {renderModalContent(modalType, data, onClose, onAddEmployee, onUpdateEmployee, setModalType, requests, onApproveRequest, onRejectRequest)}
        </div>

        {/* Footer chung - Chỉ hiện cho các loại không tự quản lý nút */}
        {modalType !== "SALARY" && modalType !== "VIEW_BY_SHIFT" && modalType !== "ADD" && modalType !== "EDIT" && (
           <div className="px-6 py-4 border-t border-gray-100 bg-white flex justify-end gap-3 sticky bottom-0">
              ...
              {(modalType === "IMPORT" || modalType === "APPROVE_ATTENDANCE") && (
 ...
 */
const renderModalContent = (type, data, onClose, onAddEmployee, onUpdateEmployee, setModalType, requests, onApproveRequest, onRejectRequest) => {
  switch (type) {
    case "ADD":
      return <AddEmployeeForm onAdd={onAddEmployee} onClose={onClose} />;
    case "EDIT":
      return <EditEmployeeForm data={data} onSave={onUpdateEmployee} onClose={onClose} onBack={() => setModalType("VIEW_BY_EMPLOYEE")} />;
    case "APPROVE_REQUESTS":
      return <ApproveRequestsContent requests={requests} onApprove={onApproveRequest} onReject={onRejectRequest} />;
    case "IMPORT":
...
const AddEmployeeForm = ({ onAdd, onClose }) => {
  const [form, setForm] = useState({ name: "", phone: "", idCard: "", checkInId: "", role: "" });
  ...
  const handleSubmit = () => {
    ...
    if (!form.checkInId.trim()) newErrors.checkInId = "Vui lòng nhập mã chấm công";
        ...
        <div className="space-y-4">
          {fields.map((field) => (
            <div key={field.name}>
              ...
              />
              {errors[field.name] && (
                <p className="text-red-400 text-[12px] mt-1">{errors[field.name]}</p>
              )}
            </div>
          ))}
          <div>
            <label className="block text-[13px] font-bold text-gray-700 mb-1.5">Chức vụ</label>
            <select
              name="role"
              value={form.role}
              onChange={handleChange}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          <div className="text-center">
            <p className="text-[13px] text-gray-500">Định dạng JPG, PNG. Tối đa 2MB.</p>
            <p className="text-[12px] font-bold text-[#5D5FEF] mt-1">{data?.staffId}</p>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="flex gap-4 mt-2">
           <div className="flex items-center gap-2 text-[13px]">
              ...
              <span className={data?.status === "off" ? "text-red-500 font-medium" : "text-gray-500"}>
