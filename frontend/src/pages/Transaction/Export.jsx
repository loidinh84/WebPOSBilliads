...
function Export() {
  // 1. Dữ liệu mẫu bám sát thiết kế (Có đổi nhẹ mã ID cho hợp lý)
  const exportInvoices = [
    { id: 'THN-20231025-001', date: '25/10/2023 14:30', supplier: 'Suntory Pepsico', total: '2,500,000', note: 'Hàng lỗi nhà SX', status: 'Chờ duyệt' },
    { id: 'THN-20231025-002', date: '24/10/2023 15:40', supplier: 'TK SAIGON', total: '1,300,000', note: 'Hết hạn sử dụng', status: 'Hoàn thành' },
    { id: 'THN-20231025-003', date: '21/10/2023 12:13', supplier: 'Aramit', total: '8,000,000', note: 'Hàng lỗi cơ học', status: 'Đã hủy' },
  ];

  // 2. Hàm xử lý màu sắc cho Badge Trạng thái
  const getStatusBadgeStyle = (status) => {
    switch (status) {
      case 'Chờ duyệt':
        return 'bg-yellow-100 text-yellow-700'; // Màu vàng
      case 'Hoàn thành':
        return 'bg-green-100 text-green-700';   // Màu xanh lá
      case 'Đã hủy':
        return 'bg-gray-200 text-gray-600';     // Màu xám
      default:
    ...
    <div className="min-h-screen bg-[#f0f2f5] font-sans text-sm text-gray-800">
      ...
      <div className="flex p-4 gap-4">
        ...
        <div className="w-[260px] flex-shrink-0 flex flex-col gap-4">
          ...
          {/* Khối Tìm kiếm (Có viền xanh như thiết kế) */}
          <div className="bg-white p-4 rounded shadow-sm border-2 border-blue-400">
            ...
            <div className="space-y-3">
              <input type="text" placeholder="Theo mã xuất hủy" className="w-full border border-gray-300 rounded px-3 py-1.5 focus:outline-none focus:border-blue-500" />
              ...
              <input type="text" placeholder="Theo người xuất hủy" className="w-full border border-gray-300 rounded px-3 py-1.5 focus:outline-none focus:border-blue-500" />
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded shadow-sm border border-gray-200">
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <input type="text" value="Toàn thời gian" readOnly className="flex-1 border border-gray-300 rounded px-3 py-1.5 bg-gray-50 text-gray-600 cursor-default" />
              </label>
          ...
          {/* Khối Trạng thái (Đã sửa lại text cho đúng logic) */}
          <div className="bg-white p-4 rounded shadow-sm border border-gray-200">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Phiếu xuất hủy</h1>
          </div>
            ...
            <div className="flex-1 overflow-auto">
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead className="bg-[#e9f0fc] sticky top-0 z-10 border-b border-gray-200">
                  {/* Trong ảnh header có màu xanh nhạt hơn, tôi dùng #e9f0fc */}
                  <tr>
                    <th className="p-3 font-semibold text-gray-800">Mã xuất hủy</th>
                    <th className="p-3 font-semibold text-gray-800">Ngày trả</th>
                    <th className="p-3 font-semibold text-gray-800">Nhà cung cấp</th>
                    <th className="p-3 font-semibold text-gray-800">Tổng giá trị</th>
                    <th className="p-3 font-semibold text-gray-800">Ghi chú</th>
                    <th className="p-3 font-semibold text-gray-800 text-center">Trạng Thái</th>
                    <th className="p-3 font-semibold text-gray-800 text-center">Thao tác</th>
                  </tr>
                </thead>
                ...
                <tbody>
                  {exportInvoices.map((invoice, index) => (
                    <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="p-3 text-gray-800">{invoice.id}</td>
                      <td className="p-3 text-gray-600">{invoice.date}</td>
                      <td className="p-3 text-gray-600">{invoice.supplier}</td>
                      <td className="p-3 text-gray-800">{invoice.total}</td>
                      <td className="p-3 text-gray-600">{invoice.note}</td>
                      
                      {/* Cột Trạng thái với thiết kế Badge */}
                      <td className="p-3 text-center">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadgeStyle(invoice.status)}`}>
                          {invoice.status}
                        </span>
                      </td>
                      ...
                      <td className="p-3 text-center">
                        <div className="flex justify-center items-center gap-3">
                          <button className="w-5 h-5 opacity-50 hover:opacity-100 cursor-pointer transition-opacity">
                            {/* Bro chèn icon con mắt (EyeIcon / View) vào đây */}
                            <img src={Icons.Eye} alt="Xem" className="w-full h-full object-contain filter brightness-0" />
                          </button>
                          <button className="w-5 h-5 opacity-50 hover:opacity-100 cursor-pointer transition-opacity">
                            {/* Bro chèn icon máy in (PrintIcon) vào đây */}
                            <img src={Icons.Printer} alt="In" className="w-full h-full object-contain filter brightness-0" />
                          </button>
                        </div>
                      </td>
                    </tr>
