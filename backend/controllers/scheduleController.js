...
const getWeeklySchedule = async (req, res) => {
  try {
    ...
    if (!startDate || !endDate)
      return res.status(400).json({ message: "Cần startDate và endDate" });
    ...
    const employeeSchedules = employees.map((emp) => {
      ...
      empShifts.forEach((s) => {
        ...
        const dayKey =
          daysMapping[new Date(parts[0], parts[1] - 1, parts[2]).getDay()];
      ...
      return {
        ...
        expectedSalary: new Intl.NumberFormat("vi-VN").format(
          Math.round(totalExpectedSalary),
        ),
      };
    });
...
const bulkUpsertShifts = async (req, res) => {
  try {
    ...
    if (!schedules || !Array.isArray(schedules)) {
      return res
        .status(400)
        .json({ message: "Dữ liệu không hợp lệ (Expect array of schedules)." });
    }
    ...
    res.json({
      success: true,
      message: `Đã cập nhật ${schedules.length} ca làm thành công!`,
    });
  } catch (error) {
