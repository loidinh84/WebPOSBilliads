...
const formatTime = (timeObj) => {
  ...
  if (timeObj instanceof Date) {
    return timeObj.getUTCHours().toString().padStart(2, "0") + ":" + 
...
const getWeeklyAttendance = async (req, res) => {
  try {
    ...
    data.forEach(row => {
      ...
      const rateMap = { "Quản lý (40k)": 40000, "Thu ngân (30k)": 30000, "Nhà bếp (30k)": 30000 };
      let rate = rateMap[CHUCVU];
      if (!rate) {
        const searchStr = (CHUCVU || "" + " " + (TENNGUOIDUNG || "")).toLowerCase();
        if (searchStr.includes("quản lý") || searchStr.includes("admin")) rate = 40000;
      ...
      if (vao_dk) {
        if (!vao_tt) {
          ...
          let notes = [];
          if (isLate) {
            const diff = (parseInt(vao_tt.split(":")[0]) * 60 + parseInt(vao_tt.split(":")[1])) - 
                         (parseInt(vao_dk.split(":")[0]) * 60 + parseInt(vao_dk.split(":")[1]));
            notes.push(`Muộn ${diff}p`);
          }
          if (isEarly) {
            const diff = (parseInt(ra_dk.split(":")[0]) * 60 + parseInt(ra_dk.split(":")[1])) - 
                         (parseInt(ra_tt.split(":")[0]) * 60 + parseInt(ra_tt.split(":")[1]));
    ...
    res.json(Object.values(staffMap).map(s => ({
      ...s,
