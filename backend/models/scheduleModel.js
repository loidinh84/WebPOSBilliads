...
const Schedule = {
  ...
  getWeekly: async (startDate, endDate) => {
    try {
      ...
      const employees = await pool.request().query("SELECT MANVIEN, TENNGUOIDUNG, CHUCVU FROM NHANVIEN WHERE DAXOA = 0 OR DAXOA IS NULL");
  ...
  upsert: async (data) => {
    try {
      ...
      await pool.request()
        ...
        .query("DELETE FROM LICHLAMVIEC WHERE MANVIEN = @manvien AND NGAY = @ngay");
      ...
      if (data.CALAM !== "OFF") {
        ...
        await pool.request()
          ...
          .input("giokt", sql.NVarChar, data.GIOKETTHUC)
          .query(`
  ...
  bulkUpsert: async (records) => {
    ...
    try {
      ...
      for (const rec of records) {
        ...
        if (rec.CALAM !== "OFF") {
          const malich = "LC" + Date.now().toString().slice(-6) + Math.floor(Math.random() * 1000);
          await req
            .input("malich", sql.NVarChar, malich)
            .input("manvien", sql.NVarChar, rec.MANVIEN)
            .input("ngay", sql.Date, rec.NGAY)
            ...
            .input("giokt", sql.NVarChar, rec.GIOKETTHUC)
            .query(`
              INSERT INTO LICHLAMVIEC (MALICH, MANVIEN, NGAY, CALAM, GIOBATDAU, GIOKETTHUC)
              VALUES (@malich, @manvien, @ngay, @calam, @giobd, @giokt)
            `);
