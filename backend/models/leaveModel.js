...
const LeaveRequest = {
  ...
  approve: async (id) => {
    ...
    try {
      ...
      const requestData = await transaction.request()
        .input("id", sql.Int, id)
        .query("SELECT MANVIEN FROM YEUCAUNGHIPHEP WHERE MAYEUCAU = @id");
      
      if (requestData.recordset.length === 0) throw new Error("Không tìm thấy yêu cầu");
      const manvien = requestData.recordset[0].MANVIEN;

      // 2. Cập nhật trạng thái yêu cầu thành 'Đã duyệt'
      await transaction.request()
        .input("id", sql.Int, id)
        .query("UPDATE YEUCAUNGHIPHEP SET TRANGTHAI = N'Đã duyệt' WHERE MAYEUCAU = @id");
  ...
  reject: async (id) => {
    try {
      ...
      await pool.request()
        .input("id", sql.Int, id)
        .query("UPDATE YEUCAUNGHIPHEP SET TRANGTHAI = N'Từ chối' WHERE MAYEUCAU = @id");
      return { success: true };
    } catch (error) {
      throw error;
    }
